...
\section{Introduction to QIT}
	...
	\section{Quantum information theory}
		...
		% AUGUST 9th 
		% methinks we don't need this section AT ALL !
		% instrument is used only is this stupid technical proof... no need to introduce here 
		% CPTP can be mentionned as a dinamical resource
		...
		\subsection{Quantum resources}	\label{subsection:quantum-resources}
			...
			% wow this girl on the table is hot. Who is the guy? They don't seem too close. what eyes... what looks...
			% should-I feel guilty if i want to sleep with her? --> naaah, Dobrinka and I are only friends
			%   -->  'tis my genetic duty to have sex with as many pretty Bulgarian women as possible!
			...
			\index{LOCC@LOCC: local operations and classical communication}
			...
			%		\hline
			%		\hline
			%		$[c\to c]$			&	noiseless bit channel.	\\
			%		$\{c\to c\}$		&	noisy classical channel, described by a pdf $p(y|x)$ \\
			%		$[cc]$				&	shared randomness  (1 rbit)	\\
			%		$[qq]$				&	shared entanglement (1 ebit) \\
			%		$[q \to q]$			&	noiseless quantum channel	\\
			%		$\{q \to q\}$		&	noisy quantum channel given by a CPTP map $\cN$ \\
			%		\hline
			%	\end{tabular}
			%	\end{center}
			%\vspace{0.10cm}
			%\TableCaptionOpt{Some resources of information theory.}{Some resources of information theory.}
			%%\caption{Table of resources of information theory}
		...
		\subsection{Distance measures}		\label{subsection:distance-measures}
			...
			\be
				TD(\rho, \sigma) 	:= \|\rho-\sigma\|_1 = \mathrm{Tr}|\rho-\sigma|
			\ee
			...
			%	We purify the density matrices $\rho$, $\sigma$ to $\ket{\Phi_\rho}$ and $\ket{\Phi_\sigma}$
			%	respectively. Then we can define:
			%	\be
			%		F(\rho,\sigma) = \mathop{ \textrm{max}}_{\stackrel{\textrm{purifications} }
			%													{ \ket{\Phi_\rho}, \ket{\Phi_\sigma}} } 
			%						\{  \quad  | \braket{\Phi_\rho}{\Phi_\sigma} |^2  \quad \}
			%	\ee 
		...
		\subsection{Ensemble and entanglement fidelity}	\label{subsection:ent-fid}
			...
			\index{i.i.d.@i.i.d.: identical, independently distributed}
			...
			\be
				\bar{F}\!\left(\E, \cN\right) :=	\sum_i	p_i F(\ket{\psi_i}, \sigma_i), 
...
%		    \end{figure}						
			...
			system. \index{entanglement!fidelity}
...
\comment{FIGURE 101: $X^n$ splits into two channels-- top left alone and bottom gets hit by E and D.}
...
\comment{FIGURE 102: State $\ket{\psi_\rho}$ splits into R (unchanged) and A. A gets hit by $\eta$.}
...
\begin{Ex}
	...
	\be
		\rho_x = \frac{1}{2} \left( \begin{array}{cc} 1&0\\0&1 \end{array} \right) = \frac{1}{2}\ket{0}\bra{0} 
			   + \frac{1}{2}\ket{1}\bra{1} 
	\ee 
	\be
		\rho_{xx} = \frac{1}{2}\ket{00}\bra{00} + \frac{1}{2}\ket{11}\bra{11} \neq \rho_x \otimes \rho_x
	\ee
\end{Ex}

%
%We say that a source iid  $\rho^{\otimes n}$ on $A^n$ can be compressed at rate $R$ if $\exists$ quantum channels
%\begin{eqnarray}
%E_n:A^n 	& \rightarrow & 		C \qquad dim(C) = 2^{nR} \\
%D_n:C 	& \rightarrow &		A^n \\
%\end{eqnarray}
%and $Pr\{X^n \neq Y^n\} \stackrel{n \to \infty}{\longrightarrow} 1$.


%\subsection{Typical Subspaces}
%Suppose $\rho = \sum_j r_j \ket{j}\bra{j}$ (spectral decomposition), define the typical projector
%\begin{eqnarray}
%P_\epsilon^{(n)} 	&=&	\sum_{j^n \in T_\epsilon^{(n)}} \ket{j_1}\bra{j_1}^{A_1}\otimes \ket{j_2}\bra{j_2}^{A_2}\otimes \ldots \ket{j_n}\bra{j_n}^{A_n}  \\
%									&=&	\sum_{j^n \in T_{\epsilon}^{(n)}} \ket{j^n}\bra{j^n}^{A^n} \qquad \textrm{$T_{\epsilon}^{(n)}$ is the typical sequences w.r.t $r$}
%\end{eqnarray}
%All the results that we proved for entropy-typical sequences are also true for frequency-typical sequences.
